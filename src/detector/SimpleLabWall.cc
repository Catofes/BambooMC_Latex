...
// anonymous namespace to register the SimpleLabWall
...
namespace {
  ...
  const bool registered = BambooDetectorFactory::Instance()->registerDetectorPart(SimpleLabWallName, createSimpleLabWall);
}

SimpleLabWall::SimpleLabWall (const G4String & name)
...
G4bool SimpleLabWall::construct()
{
  ...
  _concreteWallPhysicalVolume = new G4PVPlacement(0, G4ThreeVector(0, 0, 0), _concreteWallLogicalVolume, "ConcreteWall", _rockWallLogicalVolume, false, 0);
