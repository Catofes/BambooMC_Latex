...
PandaXSensitiveDetector::PandaXSensitiveDetector(const G4String &name, bool enableEnergyDeposition,
                                                 bool enableFlatSurfaceFlux)
        : G4VSensitiveDetector(name), _recordEnergyDeposition(enableEnergyDeposition),
          _recordFlatSurfaceFlux(enableFlatSurfaceFlux), _eDHitsCollection(0), _fsFluxHitsCollection(0)
{
...
G4bool PandaXSensitiveDetector::ProcessHits(G4Step *aStep, G4TouchableHistory *)
{
    ...
    if (_recordEnergyDeposition) {
        ...
        hit->setP(aStep->GetPostStepPoint()->GetMomentum());
        hit->setBoundry(aStep->GetPostStepPoint()->GetStepStatus() == fGeomBoundary);
