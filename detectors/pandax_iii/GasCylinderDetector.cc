...
namespace {
  ...
  BambooDetectorPart * createGasCylinderDetector ()
  {
    return new GasCylinderDetector("GasCylinderDetector");
  }

  const std::string GasCylinderDetectorName("GasCylinderDetector");

  const bool registered = BambooDetectorFactory::Instance()->registerDetectorPart(GasCylinderDetectorName, createGasCylinderDetector);
}
  ...
  : BambooDetectorPart(name)
{
  DetectorParameters dp = BambooGlobalVariables::Instance()
    ->findDetectorPartParameters("GasCylinderDetector");
...
G4bool GasCylinderDetector::construct ()
{
  ...
  _partPhysicalVolume = new G4PVPlacement(0, G4ThreeVector(_shiftX, _shiftY, _shiftZ), _partLogicalVolume, "GasCylinderDetector", _parentPart->getContainerLogicalVolume(), false, 0);
